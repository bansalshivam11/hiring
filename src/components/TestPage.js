...
export default function TestPage(props) {
  const [student_name, setStudentName] = useState("xyz");
  const [student_email, setStudentEmail] = useState("xyz");
  const [exam_id, setExamId] = useState("xyz");
  const [form_link, setFormLink] = useState("xyz");
  const [minutes, setMinutes] = useState(parseInt(123));
  const [seconds, setSeconds] = useState(parseInt(123));
  ...
  const [mobile_phone_found_count, setMobilePhoneFoundCount] = useState(0);
  const [prohibited_object_found, setProhibitedObjectFound] = useState(false);
  const [prohibited_object_found_count, setProhibitedObjectFoundCount] =
    useState(0);
  const [face_not_visible, setFaceNotVisible] = useState(false);
  const [face_not_visible_count, setFaceNotVisibleCount] = useState(0);
  const [multiple_faces_visible, setMultipleFacesVisible] = useState(false);
  const [multiple_faces_visible_count, setMultipleFacesVisibleCount] =
    useState(0);
  ...
  function update_mobile_phone_found() {
    ...
    setMobilePhoneFoundCount(mobile_phone_found_count + 1);
  }
  function update_prohibited_object_found() {
    setProhibitedObjectFound(true);
    setProhibitedObjectFoundCount(prohibited_object_found_count + 1);
  }
  function update_face_not_visible() {
    setFaceNotVisible(true);
    setFaceNotVisibleCount(face_not_visible_count + 1);
  }
  function update_multiple_faces_visible() {
    setMultipleFacesVisible(true);
    setMultipleFacesVisibleCount(multiple_faces_visible_count + 1);
  }
  /**
  ...
  return (
    <>
      ...
      <div className="name">
        <h6 align="left">
          Name: <span style={{ fontSize: "20px" }}> {student_name}</span>
        </h6>
        <h6 align="left">
          Exam ID: <span style={{ fontSize: "20px" }}> {exam_id}</span>
        </h6>
      </div>
      ...
      >
        ...
        <div>{`face not visible : ${face_not_visible}`}</div>
        <div>{`multiple face detected: ${multiple_faces_visible}`}</div>
      </div>
    </>
  );
}
